...
Компонент, хранящий наши реализации команд.

В случае, если не нашлась наша реализация, возвращает `None`.

### FunctionsExecutor

Компонент, исполняющий команду с ее аргументами.
Внутри себя обращается к `FunctionHandler`, получает вызываемый объект. Если возвращается `None`, то запускается внешняя команда через `subprocess.run`
...
`usage: grep [-h] [-i] [-w] [-A NUM] pattern [file [file ...]]`

Ищет вхождения паттерна, поддерживающего регулярные выражения. Для каждого файла выводит строки, содержащие паттерн или содержащиеся в рамках контекста (см. параметр `-A`). Если список файлов пуст, то работает с потоком ввода.

#### Параметры

`-h, --help`     выводит сообщение с помощью

`-i`             игнорирует различия в регистре

`-w`             ищет только слова целиком. Словом считается последовательность из символов `a-zA-Z0-9_`

`-A NUM`         задает ширину контекста вывода после нахождения. Например, -A 10 выведет еще 10 строк после совпадения. `NUM` должно быть неотрицательно. По умолчанию `NUM` равно 0 

### PIPE

Левоассоциативная операция, передающая поток вывода левой команды как поток ввода правой и выполняет их. В частности, из этого следует, что если одна из переданных пайпу команд будет `exit`, при ее выполнении CLI завершится. Если команда слева от пайпа завершилась с ненулевым кодом возврата, то команда справа не запускается, а в консоль будет выведен поток ошибок и вывода для запущенной левой команды.
