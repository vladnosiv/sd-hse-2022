...
def test_zero_arg():
    ...
    assert code == 0
    assert out.get_value == b''
    assert err.get_value == b''
...
def test_unexisting_path(tmp_path):
    ...
    assert code != 0
    assert out.get_value == b''
    assert len(err.get_value) > 0 #  Подставьте, что хотите :)
...
def test_happy_path(config, tmp_path):
    ...
    assert out.get_value == b''
    assert err.get_value == b''
    assert EnvironmentHandler.get_current_working_directory() == tmp_path.joinpath(*start, *argument)
